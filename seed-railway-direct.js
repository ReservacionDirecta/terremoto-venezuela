...
const SPAM = [/infinityhotel/i,/TRUSTED/i,/\.it\b/i,/https?:\/\//i,/crypto/i,/bitcoin/i,/\.com\b/i,/\.net\b/i];
function isSpam(p){return SPAM.some(r=>r.test(p.nombre+' '+p.descripcion+' '+(p.contacto||'')));}
function fp(page){return new Promise((resolve,reject)=>{https.get('https://desaparecidos-terremoto-api.theempire.tech/api/personas?page='+page+'&pageSize=50',{timeout:15000},res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{try{resolve(JSON.parse(d))}catch(e){reject(e)}})}).on('error',reject)});}

async function seed(){
  await mongoose.connect(URI,{serverSelectionTimeoutMS:15000});
  console.log('✅ Conectado a Railway MongoDB');
  const ex = await Report.countDocuments({source:'external'});
  console.log('Existentes:', ex);
  let imp=0,skip=0,spam=0,ng=0;
  for(let p=1;p<=20;p++){
    process.stdout.write('P'+p+'... ');
    let d; try{d=await fp(p);}catch(e){console.log('Error:',e.message);break;}
    if(!d?.items?.length){console.log('vacia');break;}
    let pg=0;
    ...
    console.log('+'+pg);
    if(p<20) await new Promise(r=>setTimeout(r,800));
  }
  console.log('Importados:',imp,'Saltados:',skip,'Spam:',spam,'Sin geo:',ng);
