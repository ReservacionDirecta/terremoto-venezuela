    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="flex flex-col gap-2 px-4 py-3 border-b border-border" style={{background:'var(--color-surface)'}}>
        <div className="flex items-center gap-2 flex-wrap">
          <h2 className="text-base font-bold">Personas Desaparecidas</h2>
          <span className="text-xs text-txt3">{reports.length} personas · <span className="text-blue-600">{noEnc.length} sin localizar</span> · <span className="text-green-600">{enc.length} localizados</span></span>
        </div>
        <input type="text" placeholder="Buscar por nombre o lugar..." value={q} onChange={e => setQ(e.target.value)}
               className="max-w-sm" />
        {reports.length > 0 && (
          <div className="mt-1">
            <div className="flex justify-between text-xs text-txt3 mb-1"><span>Localización</span><span>{pct}%</span></div>
            <div className="h-1.5 rounded-full overflow-hidden" style={{background:'var(--color-border)'}}>
              <div className="h-full rounded-full transition-[width] duration-500" style={{width:`${pct}%`,background:'linear-gradient(90deg,#eab308,#2563eb,#16a34a)'}} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {displayed.length === 0 && <div className="py-15 text-center text-muted">{q ? 'Sin resultados' : 'No hay desaparecidos reportados'}</div>}
      ...
      {lightbox && (
        <div className="fixed inset-0 z-500 bg-black/95 flex items-center justify-center cursor-pointer" onClick={() => setLightbox(null)}>
          ...
          <button className="absolute top-4 right-5 w-10 h-10 rounded-full bg-red-600 text-white border-none text-lg cursor-pointer font-bold"
...
function DesCard({ r, onMarcar, onFoto, fotos, setFotos }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow flex gap-3 relative"
      ...
      <span className={`absolute top-2.5 right-2.5 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${isF ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
        {isF ? 'Localizado' : 'Sin localizar'}
      </span>

      <div className="shrink-0">
        {r.fotoExterna ? (
          <img src={'/api/external/image?url=' + encodeURIComponent(r.fotoExterna)} alt="" className="w-20 h-20 rounded-xl object-cover border border-border cursor-pointer hover:scale-105 transition-transform"
               onClick={() => onFoto({foto:'/api/external/image?url=' + encodeURIComponent(r.fotoExterna),nombre:r.nombre})}
          ...
          <div className="w-20 h-20 rounded-xl border-2 border-dashed border-border2 flex items-center justify-center text-3xl text-muted">{err ? '⚠️' : ''}</div>
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-sm font-bold m-0 mr-18">{r.nombre||'Sin nombre'}</h3>
        {r.edad != null && <span className="text-xs text-txt3">{r.edad} años</span>}
        ...
        <div className="flex gap-2 mt-2 flex-wrap">
          ...
          {fotoUrl && <button onClick={() => onFoto({foto:fotoUrl,nombre:r.nombre})} className="text-xs text-blue-600 bg-transparent border-none cursor-pointer">Ver foto</button>}
        </div>
        {onMarcar && !isF && (
          <button className="w-full mt-2 inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer bg-green-50 text-green-600 border border-green-600 hover:bg-green-100 transition-colors"
